...
class RunMalletTask(Task):
    ...
    def out_malletdir(self):
        ...
        return self.outputfrominput(inputformat='featuredir', stripextension='.tok.featuredir', addextension='.malletdir')
    ...
    def run(self):
        ...
        self.ex('import-dir', keep__sequence=True, input=self.in_featuredir().path, output=self.output()[0].path)
        self.ex('train-topics', num__iterations=self.num_iter, random__seed=self.rand_var, num__topics=self.num_topics, optimize__interval=self.interval, input=self.output().path, doc__topics__threshold=self.topic_threshold, output__doc__topics=self.output().path+'.NT'+self.num_topics+'.I'+self.interval+'.THR'+self.topic_threshold+'.doc-topics', output__topic__keys=self.output().path+'.NT'+self.num_topics+'.I'+self.interval+'.rand'+self.rand_var+'.topic-keys.txt', inferencer__filename=self.output().path+'.NT'+self.num_topics+'.I'+self.interval+'.rand'+self.rand_var+'.inferencer')   
...
class RunMalletComponent(StandardWorkflowComponent):
    ...
    def accepts(self):
        return InputFormat(self, format_id='featuredir', extension='tok.featuredir', directory=True),
