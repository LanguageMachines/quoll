...
from quoll.classification_pipeline.modules.classify_ensemble import Ensemble
...
class FoldEnsemble(Task):
    ...
    def run(self):
        ...
        yield Ensemble(train=self.out_train().path,trainlabels=self.out_trainlabels().path,test=self.out_test().path,**kwargs)


class FoldsEnsemble(Task):
...
class ValidateEnsembleTask(Task):
...
@registercomponent
class RunFoldEnsemble(WorkflowComponent):
    ...
    def setup(self, workflow, input_feeds):
        ...
        kwargs = quoll_helpers.decode_task_input(['classify','validate'],[self.classify_parameters,self.validate_parameters])

        fold_ensemble_runner = workflow.new_task(
            'run_fold_ensemble', FoldEnsemble, autopass=False, 
        ...
        fold_ensemble_runner.in_directory = input_feeds['directory']
        fold_ensemble_runner.in_instances = input_feeds['instances']
        fold_ensemble_runner.in_labels = input_feeds['labels']
...
@registercomponent
class ValidateEnsemble(WorkflowComponent):
    ...
    def accepts(self):
        return [tuple(x) for x in numpy.array(numpy.meshgrid(*
            [
                ...
                InputFormat(self, format_id='featurized',extension='.tok.txt',inputparameter='instances'),
                InputFormat(self, format_id='featurized',extension='.tok.txtdir',inputparameter='instances'),
                InputFormat(self, format_id='featurized',extension='.frog.json',inputparameter='instances'),
                InputFormat(self, format_id='featurized',extension='.frog.jsondir',inputparameter='instances'),
                InputFormat(self, format_id='featurized',extension='.txtdir',inputparameter='instances'),
                InputFormat(self, format_id='featurized_docs',extension='.txt',inputparameter='instances'),
                ),
                (
                InputFormat(self, format_id='labels',extension='.labels',inputparameter='labels')
                ),
    ...
    def setup(self, workflow, input_feeds):
        ...
        if 'featurized_docs' in input_feeds.keys():
            docs = input_feeds['featurized_docs']
        else:
        ...
        if 'featurized_csv' in input_feeds.keys():
            traincsvtransformer = workflow.new_task('transformer_csv',TransformCsv,autopass=True,delimiter=self.delimiter)
            traincsvtransformer.in_csv = input_feeds['featurized_csv']
            trainfeatures = traincsvtransformer.out_features
        else:
            if 'featurized_docs' in input_feeds.keys():
                instances = input_feeds['featurized_docs']
            else:
        ...
        foldreporter.in_exp = foldrunner_ensemble.out_exp

        validator_ensemble = workflow.new_task('report_performance',ReportPerformance,autopass=True,ordinal=self.ordinal)
        validator_ensemble.in_predictions = foldreporter.out_predictions
