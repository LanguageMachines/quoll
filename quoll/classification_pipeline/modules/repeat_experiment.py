...
class RepeatExperiment(WorkflowComponent):
    ...
    def setup(self, workflow, input_feeds):
        ...
        repeater.in_testvectors = input_feeds['test']
        repeater.in_testlabels = input_feeds['testlabels']        
...
class RepeatExperimentTask(Task):
    ...
    def run(self):
        ...
        for repeat in range(self.repeats):
            yield RepeatInstance(directory=self.out_repeat_directory().path, trainvectors=self.in_trainvectors().path, testlabels=self.in_testlabels().path, testvectors=self.in_testvectors().path, trainlabels=self.in_trainlabels().path, traindocuments=self.in_traindocuments().path, testdocuments=self.in_testdocuments().path, classifier_args=self.in_classifier_args().path, i=repeat, classifier=self.classifier, ordinal=self.ordinal)
...
class RepeatInstance(WorkflowComponent):
    ...
    def accepts(self):
        return [ ( InputFormat(self,format_id='directory',extension='.repeats',inputparameter='directory'), InputFormat(self,format_id='trainvectors',extension='.vectors.npz',inputparameter='trainvectors'), InputFormat(self, format_id='trainlabels', extension='.labels', inputparameter='trainlabels'), InputFormat(self,format_id='testvectors',extension='.vectors.npz',inputparameter='testvectors'), InputFormat(self, format_id='testlabels', extension='.labels', inputparameter='testlabels'), InputFormat(self,format_id='traindocuments',extension='.txt',inputparameter='traindocuments'), InputFormat(self,format_id='testdocuments',extension='.txt',inputparameter='testdocuments'), InputFormat(self,format_id='classifier_args',extension='.txt',inputparameter='classifier_args') ) ]
...
class RepeatInstanceTask(Task):
    ...
    def run(self):
        ...
        print('Running ' + str(self.i) + 'th experiment')
