...
class RunFold(Task):
    ...
    def run(self):
        ...
        with open(self.in_labels().path,'r',encoding='utf-8') as infile:
            labels = numpy.array(infile.read().split('\n'))
...
class Run_nfold_cv(Task):
    ...
    def run(self):
        ...
        label_performance = [performance_combined[0][0]] # headers
        for i in range(1,len(performance_combined[0])-1): # labels 
            performance = [performance_combined[0][i][0]] # name of label
            for j in range(1,len(performance_combined[0][0])): # report values
                performance.append(str(round(numpy.mean([float(p[i][j]) for p in performance_combined]),2)) + '(' + str(round(numpy.std([float(p[i][j]) for p in performance_combined]),2)) + ')')
            label_performance.append(performance)
        microperformance = ['micro']
        for j in range(1,len(performance_combined[0][-1])): # micro performance
        ...
        lw = linewriter.Linewriter(label_performance)
